...
function toggleLock(vehicle)
    ...
    if hasKeys(plate) and not lockDisable then
        ...
        elseif lockStatus == 2 then
            ...
            exports['mythic_notify']:DoHudText('inform', 'Bilen er åben')
            playLockAnim(veh)
            hasToggledLock()
        else
            SetVehicleDoorsLocked(veh, 2)
                ...
                if not hasKeys(plate) and not isHotwiring and not isSearching then
                    ...
                    if IsControlJustReleased(0, 47) and not isSearching and not hasBeenSearched(plate) then -- G
                        if hasBeenSearched(plate) then
                            ...
                            Citizen.Wait(5000)
                            isSearching = false
                            exports['mythic_notify']:DoHudText('error', 'du søgte bilen igennem og du fandt ingen ting')
                        else
                            ...
                            if rnd == 4 then
                                ...
                                Citizen.Wait(3000)
                                isSearching = false
                                exports['mythic_notify']:DoHudText('error', 'du søgte i bilen der var ingen Ting')
