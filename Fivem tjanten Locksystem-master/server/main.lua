...
AddEventHandler('onyx:reqHotwiring', function(plate)
    ...
    if xPlayer.getInventoryItem('lockpick').count > 0 then
        ...
        if rnd == 20 then
            ...
            TriggerClientEvent('mythic_notify:client:SendAlert', source, {type = 'error', text = 'Din lockpick gik i stykker'})
        end
    else
        TriggerClientEvent('mythic_notify:client:SendAlert', source, {type = 'error', text = 'Du har ingen lockpicks'})
    end
end)